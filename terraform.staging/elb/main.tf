...
resource "aws_elb" "ckan-elb" {
  ...
  instances = ["${data.terraform_remote_state.nace_ckan.leader_instance_id}","${data.terraform_remote_state.nace_ckan.instance_ids}"]
...
resource "aws_route53_record" "ckan" {
  ...
  name = "catalog.ace.uaf.edu"
  type = "A"
  alias {
    name = "${aws_elb.ckan-elb.dns_name}"
    zone_id = "${aws_elb.ckan-elb.zone_id}"
